...
def add_to_store(filenames, filepaths, jd_id):
    ...
    for f_name, f_path in zip(filenames, filepaths):
        ...
        if f_path.endswith(".pdf"):
            d = models.pdf_converter.convert(f_path, meta=cur_meta)
        elif f_path.endswith(".docx"):
            d = models.doc_converter.convert(f_path, meta=cur_meta)
        else:
            raise Exception("File type not supported")
        
        # clean and split each dict (1x doc -> multiple docs)
        d = models.processor.process(d)
        sents = []
        for mind in d:
            ents = re.split('\n\n|\n',mind["text"])
            for ent in ents:
    ...
    models.document_store.write_documents(docs)
    print("HERE")
    print(models.document_store.get_all_documents(filters={"name": [filenames[0]]}))
